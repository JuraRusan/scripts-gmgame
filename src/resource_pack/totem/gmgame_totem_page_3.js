...
let { totem_base_3, fixed_slot_minecraft } = require('../Kerubifi_base.js');
let template = require('./gmgame_totem_sample_page_3.json');

const itemKeys = Object.keys(totem_base_3);
let i = 0;

const res = itemKeys.reduce((acc, curr, index) => {
    const slot = fixed_slot_minecraft[i];
    const itemTotemName = totem_base_3[curr];
...
fs.writeFileSync('./server_drop_files/resource_pack/resource_pack_totem_page_3.yml', YAML.stringify(template));
