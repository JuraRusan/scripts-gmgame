...
let { fixed_slot_minecraft, netherite_helmet_base } = require('../Shops.js');
let template = require('./shops_netherite_helmets_sample.json');

const itemKeys = Object.keys(netherite_helmet_base);
let i = 0;

const res = itemKeys.reduce((acc, curr, index) => {
    const slot = fixed_slot_minecraft[i];
    const itemShopsName = netherite_helmet_base[curr];
...
fs.writeFileSync('./src/spawn_shops/server_drop_files/shops_netherite_helmets.yml', YAML.stringify(template));