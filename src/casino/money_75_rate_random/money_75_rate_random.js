...
let { fixed_slot_four, token_code, money_75_rate_random } = require('../Casino.js');
let template = require('./money_75_rate_random_sample.json');

const itemKeys = Object.keys(money_75_rate_random);
let i = 0;

const res = itemKeys.reduce((acc, curr, index) => {
    const slot = fixed_slot_four[i];
    const itemShopsName = money_75_rate_random[curr];
...
fs.writeFileSync('./server_drop_files/casino/money_75_rate_random.yml', YAML.stringify(template));