...
let { fixed_slot_one, token_code, netherite_axe_random_base } = require('../Casino.js');
let template = require('./netherite_axe_random_sample.json');

const itemKeys = Object.keys(netherite_axe_random_base);
let i = 0;

const res = itemKeys.reduce((acc, curr, index) => {
    const slot = fixed_slot_one[i];
    const itemShopsName = netherite_axe_random_base[curr];
...
fs.writeFileSync('./server_drop_files/casino/netherite_axe_random.yml', YAML.stringify(template));