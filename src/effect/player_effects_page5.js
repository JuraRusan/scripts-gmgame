...
let { effect_slot_minecraft, effect_base_page5, idPage5, idMap, somethingIsInterfering, visibleMaterial, invisibleMaterial } = require('./Effects.js');
let template = require('./effects_sample_page5.json');

const itemKeys = Object.keys(effect_base_page5);
let i = 0;

const res = itemKeys.reduce((acc, curr, index) => {
    const slot = effect_slot_minecraft[i];
    const idMapList = idPage5[i];
    const itemEffects = effect_base_page5[curr];
...
fs.writeFileSync('./server_drop_files/player_effect/player_effect_page5.yml', YAML.stringify(template));