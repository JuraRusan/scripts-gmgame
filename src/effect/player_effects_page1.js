...
let { effect_slot_minecraft, effect_base_page1, idPage1, idMap, somethingIsInterfering, visibleMaterial, invisibleMaterial } = require('./Effects.js');
let template = require('./effects_sample_page1.json');

const itemKeys = Object.keys(effect_base_page1);
let i = 0;

const res = itemKeys.reduce((acc, curr, index) => {
    const slot = effect_slot_minecraft[i];
    const idMapList = idPage1[i];
    const itemEffects = effect_base_page1[curr];
...
fs.writeFileSync('./server_drop_files/player_effect/player_effect_page1.yml', YAML.stringify(template));